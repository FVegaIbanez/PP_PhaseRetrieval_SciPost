...
n_step = 11
length = 2048
result = Result.create(datadir_re+'0308.h5',n_px, n_step, length)
...
# for i_px in [33]:
    ...
    df_images = np.array(df_images_h5[i_px]).astype('single')

    for i_step in range(n_step):
    # for i_step in [5]:

        print(i_px, i_step)
        gs = GS(incident_wave=iw2, intensity=df_images[i_step], propergator=customized_propagator)
        if i_step != 0:
            score = []

            # df_images[i_step],xy = im.centering_com(df_images[i_step], df_images[0])
            # df_images[i_step],xy = im.centering_com(df_images[i_step], df_images[0])

            for p in np.arange(-np.pi, np.pi, np.pi/10):
            # for p in [0]:
                gs = GS(incident_wave=iw2, intensity=df_images[i_step], propergator=customized_propagator)
                ph = np.copy(result.iwph[i_px,0])
                ph[iw_data['idxmap'] == px_list[i_px]] += p
                gs.ph = ph

                gs.run(lim_diff=1e-9, lim_n=0)
                score.append(gs.error[-1])

            score.append(score[0])
            score.insert(0, score[-2])
            score = ip.normalize(score)
            minilist = np.where((score[1:-1] < score[2:]) & (score[1:-1] < score[0:-2]))[0]
            miniscore = score[minilist]
            minilist = np.arange(-np.pi, np.pi, np.pi/10)[minilist]

            # if i_step == 1:
                # minilist = [minilist.min()]
            # if abs(miniscore.min()-miniscore.max())>0.2:
            #     minilist = [minilist[miniscore.argmin()]]
            # else:
            #     last_phase = result.iwph[i_px, i_step-1] - result.iwph[i_px, 0]
            #     phase_val = np.angle( (iw*np.exp(1j * last_phase))[iw_data['idxmap']==px_list[i_px]].sum() )
            #     minilist = [minilist[ abs( 2*np.sin((minilist-phase_val)/2) ).argmin() ]]
            ...
            for p in minilist:
                ...
                gs.run(lim_diff=1e-9, lim_n=50)
                result.update(gs, i_px, i_step, p)
        else:
            gs.run(lim_diff=1e-9, lim_n=50)
            result.update(gs, i_px, i_step , 0)
